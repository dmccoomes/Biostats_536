...
#Generating alcohol and age bins
...
emmd.sp <- glm(case~s1+s2+s3+s4+s5+age+male+nonwhite+factor(educ)+BMI+packyrs+factor(smkst)+gmalcoh+badblood
               +s1:age_bin++s2:age_bin
               +s3:age_bin+s4:age_bin
               +s5:age_bin,
               data=data, family = binomial)

emmd.sp <- glm(case~s1+s2+s3+s4+s5+age+male+nonwhite+factor(educ)+BMI+packyrs+factor(smkst)+gmalcoh+badblood
               +s1:alc_bin++s2:alc_bin
               +s3:alc_bin+s4:alc_bin
               +s5:alc_bin,
               data=data, family = binomial)
...
#testing linear spline with effect modification
anova(cfmd.sp, emmd.sp, test="LRT")

anova(cfmd.lnq, emmd.lnq, test="LRT")
