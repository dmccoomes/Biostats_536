...
#DMC - changing alcohol consumption and age into bins for EM
#data$alc_bin <- cut(data$gmalcoh, c(-1, 0, 100, 200, 1000))
#data$age_bin <- cut(data$age, c(65, 75, 85, 100))

data$alc_bin[data$gmalcoh==0] <- 0
data$alc_bin[data$gmalcoh>0 & data$gmalcoh<=100] <- 1
data$alc_bin[data$gmalcoh>100 & data$gmalcoh<=200] <-2 
data$alc_bin[data$gmalcoh>200] <- 3

data$age_bin[data$age>64 & data$age<75] <- 0
data$age_bin[data$age>74 & data$age<85] <- 1
data$age_bin[data$age>84] <- 2
...
lincom(cfmd.lncu,c("dsst+dsst2+dsst3==0"))
...
lincom(emmd.lnq,c("40*dsst+40*dsst:age+40*dsst:gmalcoh+4000*dsst2+4000*dsst2:age+4000*dsst2:gmalcoh==0"))   #I don't think we need these numbers as multipliers. What are they representing?

```


#DMC - I think that we should try to add the effect modifiers in bins for age and alcohol consumption because they are hard to interpret if we include them as continuous vars
```{r effect_mod_2}
